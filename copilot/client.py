...
class CopilotClient:
    ...
    def __init__(
        ...
        self._driver = Copilot()       # consumer Copilot (copilot.microsoft.com)
        self._sydney = SydneyDriver()  # enterprise Copilot (m365.cloud.microsoft)
    ...
    def _stream_with_recovery(self, prompt, conversation_id, kwargs):
        ...
        For **enterprise** (Sydney protocol) sessions, there is no Cloudflare
        clearance to expire, so the recovery logic is skipped — the Sydney
        driver is called directly.

        For **consumer** sessions, recovery opens a *visible* browser by default:
        headless solving is unreliable on low-trust egress, so the dependable
        path is a real window (auto-clicked checkbox, or a human click). A
        headless pre-pass runs only when ``headless_clear`` is set. Recovery
        happens only before any text is emitted, so output is never duplicated.
        """
        auth = self._fresh_auth()
        protocol = (auth or {}).get("protocol")

        # Enterprise (Sydney) path: no Cloudflare, no recovery needed.
        if protocol == "sydney":
            yield from self._stream_sydney(prompt, conversation_id, kwargs, auth)
            return

        # Consumer (Copilot) path: with Cloudflare clearance recovery.
        ...
        for attempt in range(total):
            ...
            kw = dict(
                ...
                ws_url=auth.get("ws_url") if auth else None,
                **kwargs,
            )
    ...
    def _stream_sydney(self, prompt, conversation_id, kwargs, auth):
        """Drive an enterprise (Sydney protocol) turn — no Cloudflare recovery."""
        kw = dict(
            stream=True,
            proxy=self._proxy,
            cookies=auth["cookies"] if auth else None,
        ...
        for item in self._sydney.create_completion(prompt, **kw):
            yield item
