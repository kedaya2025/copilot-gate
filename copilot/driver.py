...
from .config import ATTACHMENTS_URL, BASE_URL, CHAT_WEBSOCKET_URL, CONVERSATION_URL
from .models import AbstractProvider, Conversation, ImageResponse, ImageType
from .protocol import CONSENTS_FRAME, SET_OPTIONS_FRAME
...
class Copilot(AbstractProvider):
    ...
    def create_completion(
        ...
        #
        # Enterprise accounts: the WS URL captured during login (stored as
        # ``ws_url`` in token.json) takes priority over the config default, so
        # the driver hits the exact endpoint the browser used.
        base_ws_url = ws_url or self.websocket_url
        websocket_url = f"{base_ws_url}&clientSessionId={uuid.uuid4()}"
        if access_token:
