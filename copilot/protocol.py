...
from .config import CHAT_WEBSOCKET_URL

# Re-exported for backward compatibility. The actual value comes from
# copilot.config so it can be overridden via the COPILOT_URL env var.
# The driver prefers a URL captured live during login (stored in token.json
# as ``ws_url``) over this default.
