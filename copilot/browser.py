...
from .config import CHAT_PAGE_URL, COOKIE_DOMAINS
from .useragent import CHROME_UA

COPILOT_URL = CHAT_PAGE_URL
...
class BrowserCopilot:
    ...
    def __init__(
        ...
        self._captured_ws_url: Optional[str] = None
        self._captured_protocol: Optional[str] = None  # "copilot" or "sydney"
    ...
    def login(self, path: str = DEFAULT_AUTH_FILE, timeout: int = 300) -> dict:
        ...
        print(
            f"\nA browser window is open at {COPILOT_URL}.\n"
    ...
    def _install_ws_listener(self) -> None:
        ...
        **Consumer Copilot** opens ``wss://.../c/api/chat?...&accessToken=<token>``
        (plus, for federated logins, ``&X-UserIdentityType=google``).

        **Enterprise Copilot** (Microsoft 365) opens
        ``wss://substrate.office.com/m365Copilot/Chathub/<oid>@<tid>?...&access_token=<JWT>``
        and uses the Sydney/Bing Chat protocol (``type:1, target:"update"`` with
        ``writeAtCursor``) instead of the consumer's ``appendText``.
        ...
        def on_ws(ws):
            try:
                url = ws.url
                # Consumer: match /c/api/chat; Enterprise: match /Chathub/
                is_consumer = "api/chat" in url
                is_enterprise = "Chathub" in url
                if not (is_consumer or is_enterprise):
                    return

                q = parse_qs(urlparse(url).query)

                if is_enterprise:
                    # Enterprise: token is in access_token (underscore).
                    tok = (q.get("access_token") or [None])[0]
                    # Store the WS base URL (path only, no query) — the driver
                    # appends its own session/conversation params.
                    self._captured_ws_url = url.split("?")[0]
                    self._captured_protocol = "sydney"
                else:
                    # Consumer: token is in accessToken (camelCase).
                    tok = (q.get("accessToken") or [None])[0]
                    self._captured_ws_url = url.split("?")[0] + "?api-version=2"
                    self._captured_protocol = "copilot"
    ...
    def _on_chat_frame(self, payload) -> None:
        ...
        Consumer Copilot streams ``appendText`` (or ``imageGenerated``) frames;
        enterprise (Sydney protocol) streams ``writeAtCursor`` inside
        ``type:1,target:"update"`` frames. Either means the warm-up reply is
        flowing, i.e. the gate passed — auto_clear's success signal."""
        try:
            data = payload if isinstance(payload, str) else bytes(payload).decode("utf-8", "ignore")
        except Exception:
            return
        if "appendText" in data or "imageGenerated" in data or "writeAtCursor" in data:
    ...
    def _clearance_value(self) -> Optional[str]:
        ...
        actually earned (:meth:`auto_clear` waits on it). The cookie may be set
        on ``.copilot.microsoft.com`` (consumer) or ``.cloud.microsoft``
        (enterprise), so we scan all cookies for it."""
    ...
    def cookies(self) -> Dict[str, str]:
        """Return the signed-in cookies as a name->value dict.

        Consumer cookies live on ``*.microsoft.com``; enterprise cookies may
        also live on ``*.cloud.microsoft``. We accept both so the driver gets
        every cookie the browser earned.
        """
        self._ensure_started()
        try:
    ...
    def export_auth(self, path: str = DEFAULT_AUTH_FILE, stamp: Optional[float] = None) -> dict:
        ...
        auth = {
            ...
            # The actual WebSocket endpoint captured off the page's own chat
            # socket. The driver prefers this over the config default so it hits
            # the exact URL the browser used (enterprise uses a different domain
            # and possibly a different path prefix).
            "ws_url": self._captured_ws_url,
            # "copilot" (consumer) or "sydney" (enterprise / Microsoft 365).
            # The driver uses this to pick the right WebSocket protocol.
            "protocol": self._captured_protocol,
